...
if (empty($_SERVER['HTTP_REFERER']) || empty($downurl)) {
    exit(json_encode(['code' => 400, 'msg' => '升级失败，请勿从非法页面打开！'], true));
}
if (empty($_COOKIE['admin_name']) || $_COOKIE['admin_name'] !== $_GET['name']) {
    exit(json_encode(['code' => 400, 'msg' => '升级失败，您目前是未登陆状态！'], true));
}

$file = httpcopy($downurl);
if ($file == false) {
    exit(json_encode(['code' => 201, 'msg' => '升级失败，无法下载升级包文件！请尝试手动下载文件覆盖更新'], true));
}
...
if ($zip->open($file) === true) {
    ...
    $zip->close();
    unlink($file);
    echo json_encode(['code' => 200, 'msg' => '升级成功！如果有问题，请及时反馈！'], true);
} else {
    echo json_encode(['code' => 201, 'msg' => '升级失败，无法解压缩升级包文件，请尝试手动下载文件覆盖更新！'], true);
}
