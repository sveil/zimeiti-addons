<?php
    ...
    if (empty($path) || !$mac_ver) {
        echo showTips('苹果cmsV10主程序文件缺失', '请检测苹果cms主程序文件，或者重新安装主程序<br><a href="https://github.com/magicblack/maccms10" target="_blank">点此下载新版程序</a>');
        die;
    }
    if (strpos($_SERVER['HTTP_REFERER'], 'load.html?flag=vod')) {
        echo showTips('未检测到断点数据', '程序中没有断点记录，无法执行断点采集！<br>如果你清理过程序缓存，也会将断点记录清除掉！');
        die;
    }
    if (!is_writable('./')) {
        echo showTips('/addons/mycj/ 没有写入权限', '请开放mycj文件夹的读取写入权限');
        die;
    }
    if (!is_writable('./cache')) {
        echo showTips('文件夹写入权限不足', '请调整 mycj/cache/ 文件夹的写入权限');
        die;
    }
    ...
    if (!file_exists('./cache/data.php') || !file_exists('./cache/faves.php') || !file_exists('./cache/user.txt')) {
        echo showTips('文件缺失', 'addons/mycj/cache文件夹内缺少文件');
        die;
    }
    $faves = [];
    ...
    if (PHP_VERSION < '5.5') {
        echo showTips('PHP版本不符合插件运行要求，当前PHP版本' . PHP_VERSION . '', '推荐使用php7.0以上环境！');
        die;
    }
    if (stristr($_SERVER['HTTP_USER_AGENT'], 'MSIE') || stristr($_SERVER['HTTP_USER_AGENT'], 'Trident')) {
        echo showTips('插件不支持在IE内核下使用', '请调整浏览器内核，或者使用谷歌浏览器、360浏览器等极速模式');
        die;
    }
    $userid = '';
...
<form class="layui-form">
    <div class="layui-tab">
        <div class="layui-btn-group">
			...
			<a href="javascript:;" target="_blank" class="layui-btn qqlink"><i class="layui-icon">&#xe63a;</i>插件交流群</a>
        </div>
...
<div class="layui-tab layui-collect"><center>数据加载中，请稍后....<br>如果长时间加载不出来，请刷新重试<br>或者联系QQ：209910539反馈</center></div>
<div class="layui-footer footer">
  <div class="layui-main">
    <p>© 2018 - 2020 <a href="http://www.vrecf.com/index.html?from=mycjv10">萌芽模板网</a> 收集整理制作 联系QQ：209910539</p>
    <p>免责申明：本插件收集整理的资源站均来源于网络，本插件不存储视频链接、不提供视频数据，不对资源站提供的数据负责！</p>
...
<script src="https://caiji-api.oss-accelerate.aliyuncs.com/v10.3.1/main.min.js?v=<?php echo date('Hi'); ?>"></script>
<script src='https://caiji-api.oss-accelerate.aliyuncs.com/js/myad.js?v=<?php echo date('Hi'); ?>'></script>
...
<script src='./js/mycj.min.js?v=<?php echo date('Hi'); ?>'></script>
<script src="https://caiji-api.oss-accelerate.aliyuncs.com/V10/update.js?v=<?php echo date('Hi'); ?>"></script>
<script src="https://caiji-api.oss-accelerate.aliyuncs.com/v10.3.1/mycjv10.js?v=<?php echo date('Hi'); ?>"></script>
</body>
</html>
